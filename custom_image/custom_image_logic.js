...
style.textContent = `
  ...
  .layers-scroll { height: 400px; overflow-y: auto; flex: 1; border: 1px solid #555; }
  .layers-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 5px; padding: 5px 5px 5px 5px; }
...
async function render_image_layers(divId, targetId) {
  ...
  try {
    ...
    const hasStickerLayers = Array.isArray(entry.tags) && entry.tags.includes('custom_image');
